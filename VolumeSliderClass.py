...
class VolumeSlider:  # Creates the class used for the volume slider button
    def __init__(self, position_y, type, screen, music_volume, master_volume):  # The simple base conditions
    ...
    def hover(self, mouse_position_x, mouse_position_y):
        if 1050 <= mouse_position_x <= 1050 + 250:  # Detects if mouse position x is within set bounds
            if self.PositionY + 20 <= mouse_position_y <= self.PositionY + 160:  # Detects if mouse y is in bounds
        ...
        if 50 <= mouse_position_x <= 50 + 250:  # Detects if mouse position x is within set bounds
            if self.PositionY + 20 <= mouse_position_y <= self.PositionY + 160:  # Detects if mouse y is in bounds
                pygame.draw.rect(self.ScreenToAddTo, (80, 80, 80),
    ...
    def button_press_checker(self, volume, mouse_position_x, mouse_position_y):
        if 1050 <= mouse_position_x <= 1050 + 250:  # Detects if mouse position x is within set bounds
            if self.PositionY <= mouse_position_y <= self.PositionY + 160 and volume < 100:  # Detects if mouse y
                if self.Type == "Music":
                    volume = volume + 20
                    return volume
        ...
        if 50 <= mouse_position_x <= 50 + 250:  # Detects if mouse position x is within set bounds
            if self.PositionY <= mouse_position_y <= self.PositionY + 160 and volume > 0:  # Detects if mouse y
                if self.Type == "Music":
                    volume = volume - 20
                    return volume
